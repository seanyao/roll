...
| Story | Description | Status |
|-------|-------------|--------|
| [US-LOOP-005](docs/features/loop-write-integrity.md#us-loop-005) | 每个 cycle 跑完都留下结束记号 — 把所有结束路径（合并成功、孤儿恢复、超时、崩溃自愈、PR 失败兜底）都补上结束信号，dashboard 不再把早已结束的 cycle 显示成"还在跑" | ✅ Done |
