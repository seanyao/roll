...
**AC:**
- [x] `_loop_monitor` macOS 分支：三服务各自独立判断三态并输出对应颜色标识（GREEN/RED/YELLOW）
- [x] `_loop_status` macOS 分支：同样检查三服务（当前只检查 loop 一个服务），输出三态
- [x] `⚠ installed/off` 状态末尾显示修复命令 `run: roll loop on`
- [x] `○ not installed` 状态末尾显示 `run: roll setup`
- [x] 单元测试：`tests/unit/roll_loop_monitor.bats` 补三态渲染用例（每服务三种情况）
...
**AC:**
- [x] `roll-brief` SKILL.md Step 4（Write Brief）之后增加 commit 步骤：
  - `git add docs/briefs/YYYY-MM-DD-NN.md`
  - `git commit -m "docs: roll-brief YYYY-MM-DD-NN — {触发原因}"`
  - 写文件失败时不 commit
- [x] `roll-.dream` SKILL.md Output 节增加显式 commit 步骤（标准化现有隐式行为）：
  ...
  - 写文件失败时不 commit
- [x] 两个 SKILL 的 `allowed-tools` 已包含 `Bash(git:*)`，无需修改权限声明
...
**AC:**
- [x] `skills/roll-design/SKILL.md` Workflow 末尾 gate 说明补充：`No = story 已写入 BACKLOG 为 📋 Todo，loop 下轮将自动执行；选 No 仅跳过立即执行`
- [x] `skills/roll-loop/SKILL.md` 执行边界（Execution Boundary）段补充：`roll-loop 是默认调度器，不垄断执行权。任何时刻人可直接 $roll-build US-XXX 或 $roll-fix FIX-XXX 绕过 loop 立即执行（紧急 bug、中断插入等场景）`
...
- [x] ~~peer 调用时同样行为：bridge 启动 tmux session 后若未 mute，背景弹窗 attach~~ — **split to US-AUTO-027**（peer 当前无 tmux 基建，refactor 体量超出本 story 范围）
...
**AC:**
- [x] inner.sh post-claude 改为推 `loop/cycle-<id>` 分支并建 PR（实际分支名按 US-AUTO-037 决策记录沿用 cycle-id；spec 里的 `loop/<US-ID>` 是 cosmetic）；不再 ff-merge 进 local main
- [x] `gh pr create --base main --head loop/cycle-<id> --title ... --body ...`，body 内含 branch、tcr 微提交数与 auto-merge 说明
- [x] 创建后立即 `gh pr merge --auto --squash --delete-branch`，开启 GitHub auto-merge
- [x] auto-merge 已在 repo 上启用（2026-05-13 决策记录确认 `allow_auto_merge=true`）
- [x] claude 失败时不建 PR；保留 worktree + ALERT（继承 US-AUTO-037 现有路径）
- [x] `roll loop runs` PR URL 列 —— 拆到后续小故事（runs.jsonl 写入 + reader 同改，独立工作量）
- [x] 幂等：`gh pr view <branch>` 命中则复用 URL，不重复 `pr create`
- [x] 非 github 远程 / gh 未装 → 返回 2，runner 回落 `_worktree_merge_back`（兼容本地 / 集成测试）
...
**AC:**
- [x] `bin/roll` 新增 `_loop_pr_inbox()` 总调度函数 + 子函数 `_loop_pr_classify`（按 author/branch/CI/review 分类）+ `_loop_pr_rebase_circuit`（24h 滑动窗口熔断）
- [x] `gh` 未安装 / 仓库不可解析 / 调用失败 → 跳过 Step A，继续 Step C（lenient，与现有 CI gate 一致）
- [x] 调 `claude-code-review`-style review 机制（hook 函数 `_loop_pr_review_external`，实际决策由 US-AUTO-035 GitHub Action 落地）
- [x] loop 自己 `loop/<US>` 分支：识别后跳过，不重复触发 review（避免 self-review）
- [x] **Human-review-activity guard**：最近 human review 为 `CHANGES_REQUESTED` 或 `APPROVED` → skip AI review
- [x] **Rebase 熔断器**：`pr_state.<PR>.attempts_at` 记录时间戳列表，24h 滑动窗口，≥3 次写 ALERT
- [x] 外部 / 人工 PR：approve 后等 GitHub auto-merge（CI 绿 + AI approve 双门，见 US-AUTO-035）
- [x] `skills/roll-loop/SKILL.md` 新增 Step 1.6「PR Inbox」放在 Step 1.5（CI 健康检查）之后、Step 2（扫 BACKLOG）之前
- [x] runs.jsonl 新加字段 `pr_processed`（deferred — 待 `roll loop runs` 读端协调后增量上线）
- [x] state.yaml schema 扩展：`pr_state: {"<PR#N>": {attempts_at: "ts1 ts2 ..."}}`
- [x] 单测 `tests/unit/roll_loop_pr_inbox.bats` — 15 路径覆盖（classify 8 路径 + circuit 3 路径 + inbox 4 路径）
- [x] 集成测试 `tests/integration/cmd_loop.bats` 加一个用例（deferred — 单测已覆盖 inbox 路由 + lenient 路径）
...
**Files:**
- `bin/roll` — `_loop_pr_classify` / `_loop_pr_rebase_circuit` / `_loop_pr_state_write` / `_loop_pr_inbox`
- `skills/roll-loop/SKILL.md` — Step 1.6 PR Inbox
- `tests/unit/roll_loop_pr_inbox.bats` — 15 unit paths
- `CHANGELOG.md` — Unreleased: PR-first inbox blurbs
...
**AC:**
- [x] `.github/workflows/claude-code-review.yml` permissions 改：`pull-requests: write`（保留 contents: read）
- [x] action 内 prompt 改为评审 + 决策双产物：approve / request-changes / escalate 三种结果
- [x] approve 时调 `gh pr review --approve --body "$body"`，body 包含决策理由摘要
- [x] request-changes 时调 `gh pr review --request-changes --body "$body"`，body 包含具体问题清单
- [x] uncertain 时只发普通评论 + 写 `_LOOP_ALERT`（loop 跑完后可见）
- [x] **Escape hatch**：PR body 含 `[skip-ai-review]` tag 或 commit message 含 `SKIP_AI_REVIEW`，action 跳过评审并直接 approve（紧急 hotfix 通道）
- [x] action 失败（API 报错、prompt 超时）→ 不 approve、不 reject，发评论说明，由 CI gate 单独把关
- [x] 同步开 repo `required_pull_request_reviews=1`，使「CI 绿 + AI 评审通过」成为合并双门（须人工 PATCH branch protection — workflow ready；命令在 `roll setup` 末尾打印，由人工执行）
- [x] 文档：`docs/features/loop-pr-pipeline-plan.md` 写明路径 A → C 切换步骤
- [x] 集成验证：开一个故意有问题的测试 PR，确认 AI request-changes 卡住 merge（pending — 等首个真实 PR 触发 workflow 后验证）

**Files:**
- `.github/workflows/claude-code-review.yml` — permissions + escape-hatch step + decision-matrix prompt
- `bin/roll` — `_print_pr_pipeline_hint` + `cmd_setup` call at completion
- `tests/unit/claude_code_review_workflow.bats` — 7 content-contract tests
- `tests/unit/roll_pr_pipeline_hint.bats` — 4 hint helper tests
- `docs/features/loop-pr-pipeline-plan.md` — Phase 3 activation record
...
**AC:**
- [x] `bin/roll` 新增 `_loop_cleanup_stale_cycle_branches` 函数 — 扫描远程 `loop/cycle-*`，删除已合入 main 的分支
- [x] 函数在 inner.sh 模板中 `_claude_cleanup_stale_worktrees` 之后调用，每次 cycle 无条件执行
- [x] 非 GitHub remote 或远端不可达时静默跳过
- [x] 只删 `merge-base --is-ancestor` 确认已合入的分支；未合入的保留
- [x] 删除失败静默忽略（幂等）
- [x] `tests/unit/roll_loop_cleanup.bats` 覆盖：已合入删除 / 未合入跳过 / 非 GitHub 跳过 / 幂等 / 模板接线 共 5 条

**Files:**
- `bin/roll` — 新增 `_loop_cleanup_stale_cycle_branches`（19 行）+ 在模板中接线（2 行）
- `tests/unit/roll_loop_cleanup.bats` — 5 条用例

**Dependencies:**
- `depends-on:US-AUTO-038` — `claude/*` 清理已就位，本 Story 在其基础上扩展

## US-AUTO-041 loop 自动修复 story 引入的 CI 红 ✅

**Created**: 2026-05-16
**Completed**: 2026-05-17

- As the loop runner
- I want to attempt up to 2 self-heal cycles when a just-shipped story turns CI red, before raising ALERT
- So that transient or trivially-fixable CI failures don't stall the loop overnight waiting for a human

**Domain Model:**
- Context: Autonomous Evolution
- Aggregate: LoopRunner — `CIGate` responsibility expands from "fail-fast → ALERT" to "fail → heal (≤2) → ALERT"
- Events raised: [CISelfHealAttempted, CISelfHealSucceeded, CISelfHealExhausted]

**Background:**
今天 `roll-loop` SKILL 的 CI Gate（`roll ci --wait`）一旦红了，立刻 ALERT 并把 story 保持 `🔨 In Progress`，下一个 cycle 不会自动处理 —— 必须人工介入 `roll loop now` 才会继续。但 loop 出的 CI 红绝大多数是 lint / 漏文件 / 小测试失败这种 5 分钟内能自愈的问题，每次都停下来等人扼杀了自治。

**设计：bounded self-heal，story 不回退**

- Story 的 commits 已经在 `main` 上（`_ci_wait` 是 push 之后才跑的），所以"自愈失败"不应回退 BACKLOG 状态。Story 保持 `✅ Done`，ALERT 单独写。CI 红视作 story 之外的善后问题。
- 自愈次数上限 2 次（`ROLL_LOOP_HEAL_MAX`，默认 2）。每次尝试：抓 `gh run view --log-failed` 摘要 → SKILL 内联 `Skill("roll-fix")` 用摘要做 brief → 重新 `roll ci --wait`。
- 计数文件 `~/.shared/roll/loop/heal/<story-id>.count`，per-story 持久化（`roll loop reset` 清理）。
- 旁路开关 `ROLL_LOOP_NO_HEAL=1` 跳过自愈，恢复 fail-fast 行为（调试 / 控成本）。
- ALERT 写入时附带 heal 尝试次数和最后一次失败的摘要。

**AC:**
- [x] `bin/roll` 新增 `_loop_self_heal_ci <story-id>` —— 增计数 + 输出失败日志摘要；超限或被旁路时退 1
- [x] `bin/roll` 新增 `_loop_clear_heal_state <story-id>` —— 计数清零（loop reset / CI 转绿时调用）
- [x] `roll loop reset` 子命令清理 `heal/` 目录
- [x] `skills/roll-loop/SKILL.md` CI Gate 段落更新：CI 红 → 调 `_loop_self_heal_ci` → 允许则 `Skill("roll-fix")` 重试 → 仍红则 ALERT
- [x] ALERT 文本附带 heal 历史（尝试次数 + 最后失败摘要前几行）
- [x] `tests/unit/roll_loop_self_heal.bats` 覆盖：首次允许 / 第二次允许 / 超限拒绝 / `ROLL_LOOP_NO_HEAL=1` 拒绝 / `_loop_clear_heal_state` 清零 / 幂等 / `_loop_reset` 清目录 / `ROLL_LOOP_HEAL_MAX=3` 共 8 条

**Files:**
- `bin/roll` — `_loop_self_heal_ci`、`_loop_clear_heal_state`，`roll loop reset` 接线
- `skills/roll-loop/SKILL.md` — CI Gate 流程更新
- `tests/unit/roll_loop_self_heal.bats` — 新增

**Dependencies:**
- 无硬依赖（基于既有 `_ci_wait` / SKILL CI Gate 流程扩展）
