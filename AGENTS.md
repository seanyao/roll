...
> **`main` 即 v3：roll 的引擎是 TypeScript（`packages/`），bash 留作回落引擎。**
> **`main` is v3: the engine is TypeScript (`packages/`); bash stays as the fallback engine.**
> 按"反馈闭环 + 能力域分层"收敛。给在本仓干活的 agent / loop 读。v2 归档在 `v2` 分支（锚点 tag `v2-freeze-2026-06-04`）。
> 规格依据：`.roll/archive/v3-planning/README.md`（包地图 + 冲突裁定表）；每 story 纪律：`.roll/archive/v3-planning/build/prompts/p2-build-cycle.md`。
...
| 范围 | 状态 |
|---|---|
| **走 TS** | `status` · `agent`（view/list/use/set/unknown 全 TS；use 锁 easy/default/hard 并同步 `.roll/local.yaml`，set 写 `.roll/agents.yaml`；无 bash 回落）· `config`（全面 TS，US-PORT-006：读面 + 写面 + 三个紧凑 facade（loop-window/loop-schedule/dream-time），无 bash 回落；**有意分歧（白名单）**：config 写入不再隐式重挂 launchd（应用新调度走 `roll loop on`，facade 仍打印该提示），CLI 输出与 v2 逐字节一致；并修复 v2 `_config_resolve` 在全局 config.yaml 缺失时的 `set -u` 未绑定变量崩溃）· `prices`（show/help/unknown/refresh 全 TS；refresh 原生 vendor registry + parser + snapshot writer，无 bash 回落）· `backlog`（显示）· `loop status` · `loop run-once`（v3 runner，含 --dry-run）· `dream`（US-PORT-020：`run-once` 为 dream 服务 v3 心脏，其他参数冻结为 v2 unknown-command 面；无 bash 回落）· `loop on|off|pause|resume`（US-LOOP-009 调度面：`on` 生成 v3 自包含 runner——wrapper 直调 run-once；**有意分歧（白名单）**：弃 v2 tmux 外/内层与引擎 source；US-PORT-008 起 dream 服务也由 `on` 生成 v3 自包含 runner（同形：PATH 引导 + PAUSE marker + 直调 `roll dream run-once`，每日调度），退役 FIX-197 的断链僵尸 runner；含 v2 小时窗 printf 八进制潜伏 bug 修复）· `loop eval` · `loop story` · `loop runs`（含 --all/--detail）· `loop signals`（US-PORT-007：四个读面薄读取，eval/story 复用 dashboard cycle 管线、runs 直读 runs.jsonl、signals 走 core detectSignals，无 bash 回落）· `loop monitor` / `loop attach`（US-PORT-007 退役：v3 runner 已把每周期跑进 tmux session roll-loop-<slug>，两者改为单语重定向 stub，不再跑 v2 tmux 弹窗）· `lang` · `skills` · `alert` · `doctor` · `changelog`（确定性：generate --no-ai/--json/--write、help、未知；默认 AI 润色仍回落 bash）· `consistency` · `feedback` · `init`（确定性脚手架 + roll-init.py v2 UI 原生重写；legacy onboard 由 TS 直接起 agent；--apply/onboard changeset/未知 -flag/无模板守卫均 TS；无 bash 回落）· `offboard` · `migrate` · `setup`（全 6 步管线 + roll-setup.py v2 UI 原生重写；含无 conventions 源守卫；无 bash 回落）· `update`（npm/curl 双升级路径 + 缓存失效 + post-update setup 链 + changelog；npm/curl/tar 经 spawn，curl 原子目录 swap 是唯一白名单缺口）· `test`（隔离派发器：仅 type=none，--where 路由 / --reset 锁+派发 / 默认 exec 经 forward 的 npm test 跑宿主；未知类型（含残留 tart 配置，REFACTOR-046 已删该 lane）显式报错 exit 1，绝不静默回落宿主；help 与 unknown-exec 清单为白名单分歧）· `ci`（读面：gh 缺失 warn / 非 git 仓 err / run-list 失败 / 无记录提示 / 逐 run 状态列表；--wait CI gate 的 _ci_wait 轮询回落 bash）· `slides`（渲染器 + 校验器原生重写，逐字节对齐冻结 python oracle：build / list / preview / logs / templates / delete --force / help / 派发；`new` 走 AI agent 生成 deck → 回落 bash，交互式 `delete` 确认 → 回落 bash） |
| 其余子命令 | **bash 回落**（经 `@roll/cli` 桥接，stdio/退出码透传） |
| 引擎 | ✅ 第 0-4 层 100%（地基 5 + CLI 7 + 领域服务 13 + infra 6 + loop 8）；**US-LOOP-006 真实并行验证 3/3 PASS**（v2 done ≡ v3 delivered，真 claude 双腿）；941+ TS 测试全绿，已 port 命令全部转为冻结期望断言（US-PORT-009e），测试期零旧引擎 spawn |
...
## 5. v2 = 标准答案（冻结期望，oracle 已卸任）

`v2` 分支（bash v2，锚点 tag `v2-freeze-2026-06-04`）是只读快照、行为标准答案。**US-PORT-021 后：bash 引擎 `bin/roll` 已从本仓删除**——CLI 全 TS-native，未知命令直接打 usage（不再回落 bash）。bats 套件已随 US-CUT-002 整体退役（diff-test 是现行守护；v2 分支仍保有当年的 bats 全量）。**所有已 port 命令的 difftest 均为冻结期望断言（inline snapshot / 确定性自校验），测试期不再 spawn `bin/roll` / `python3` / `jq` 等旧引擎**；`bridge.test.ts` 校验 TS 路由本身。每条迁移 story：先读懂对应 v2 实现的可观察契约（入参/stdout/副作用/退出码/写的文件与事件），TS 版复刻其行为。**行为对齐优先于代码美观。** 写之前先查 `.roll/archive/v3-planning/build/invariants.md` 对应域，把当年的坑当 diff-test/chaos 带走。
...
`main` 已是默认分支，`@seanyao/roll` 发 TS-first CLI（v3.0.0 起）。v2 在 `v2` 分支（锚点 tag `v2-freeze-2026-06-04`）留一键回滚。子命令已全部迁完；US-PORT-021 删除了 `bin/roll` 回落引擎，CLI 现为纯 TS。
