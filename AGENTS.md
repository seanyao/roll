...
语言规则遵循 **single-language surfaces with multilingual resources**：每个 artifact 一次只渲染一种语言，多语资源藏在 locale/catalog 边界之后，不在同一块表面混排。

- **Owner conversation**：用用户语言回答，不自动追加翻译。
- **Agent contract / skill / convention**：英文，技术标识符、命令、路径、包名保持原文。
- **CLI output**：跟随解析后的 `locale`，`en` 或 `zh` 单语呈现，不混排。
- **HTML projection / site**：无 JS 默认也只显示一种语言；多语言资源可存，但同一时间只渲染一种语言。
- **Docs / guide / release note**：按文件或目录分语言（`guide/en` vs `guide/zh`），单语文件。
- **Backlog / spec**：项目主语言（本仓为中文），ID、命令、类型名保持原文。
- **Code / comments**：英文，仅用户-facing 文案和测试 fixture 可保留原语言引用。

详见 `.roll/features/i18n/language-surface-policy-plan.md`。
