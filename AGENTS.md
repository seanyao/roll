...
| 范围 | 状态 |
|---|---|
| 全部子命令 | **bash**（经 `packages/cli` 桥接回落 `bin/roll`，stdio/退出码透传） |
| 桥接层 | ✅ `@roll/cli` dispatch：TS-first + bash fallback（US-SCAF-004） |
| i18n | ✅ `@roll/spec` t()/resolveLang + v2 全量 catalog 582 keys，en/zh diff-test 绿（US-SCAF-001） |
