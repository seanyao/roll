...
| 范围 | 状态 |
|---|---|
| **走 TS** | `status` · `agent list` · `config`（读面）· `prices`（show/help）· `backlog`（显示）· `loop status` · `loop run-once`（v3 runner，含 --dry-run）· `lang` · `skills` · `alert` · `doctor` · `changelog`（确定性：generate --no-ai/--json/--write、help、未知；默认 AI 润色仍回落 bash）· `consistency` · `feedback` · `init`（确定性脚手架 + roll-init.py v2 UI 原生重写；legacy onboard / --apply / 未知 -flag / 无模板回落 bash）· `offboard` · `migrate` |
