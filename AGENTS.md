...
`v2` 分支（bash v2，锚点 tag `v2-freeze-2026-06-04`）是只读快照、行为标准答案；同一份 bash 也在本仓的 `bin/roll` 内随回落引擎一起发布。bats 套件已随 US-CUT-002 整体退役（diff-test 是现行守护；v2 分支仍保有当年的 bats 全量）。**US-PORT-009e 后：所有已 port 命令的 difftest 已转为冻结期望断言（inline snapshot），测试期不再 spawn `bin/roll` / `python3` / `jq` 等旧引擎**；`bridge.test.ts` 保留对 bash 回落桥接本身的字节级校验（合理的白名单例外）。每条迁移 story：先读懂对应 v2 实现的可观察契约（入参/stdout/副作用/退出码/写的文件与事件），TS 版复刻其行为。**行为对齐优先于代码美观。** 写之前先查 `.roll/archive/v3-planning/build/invariants.md` 对应域，把当年的坑当 diff-test/chaos 带走。
