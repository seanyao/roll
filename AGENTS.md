...
| 范围 | 状态 |
|---|---|
| **走 TS** | `status` · `agent list` · `config`（读面）· `prices`（show/help）· `backlog`（显示）· `loop status` · `loop run-once`（v3 runner，含 --dry-run）· `lang` · `skills` · `alert` · `doctor` |
| 其余子命令 | **bash 回落**（经 `@roll/cli` 桥接，stdio/退出码透传） |
| 引擎 | ✅ 第 2 层领域服务 13/13 + 第 3 层 infra 6/6 + 第 4 层 loop 7/8 已 port（差 US-LOOP-006 并行验证=需真实运行）；831 个 TS 测试全绿，diff-test 逐字节对齐冻结 oracle |
