...
| [US-AUTO-038](docs/features/autonomous-evolution.md#us-auto-038) | 清理孤立 `claude/*` session 分支 — Claude Code agent session 结束后遗留的 `claude/*` 分支堆积在 GitHub；在 `inner.sh` 里 claude 启动前记快照、退出后 diff 并立即删除本次新增的 `claude/*` 远端分支，事件驱动不做定期扫描 `Domain: Autonomous Evolution > LoopRunner > BranchHygiene` `depends-on:US-AUTO-033` | 📋 Todo |
