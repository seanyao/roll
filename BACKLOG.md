...
| REFACTOR-021 | release pipeline 双管线统一 — `roll update` 在 CLI 显示的 changelog（`_show_changelog` at bin/roll:5002 直读 CHANGELOG.md）和 GitHub release 页面显示的 changelog（`scripts/release.sh:_run_release_notes_skill` 用 AI 重写后写入 `release_notes.txt`）走两条独立路径，导致格式漂移：CHANGELOG.md 自身在 v2026.514.3/.514.2 是 `###` 分组、v2026.514.5/.515.1 退化为扁平 `**New**/**Fixed**` 列表，CLI 原样照搬故飘；同时 `release_notes.txt` 含两类脏字符 —— (a) `scripts/release.sh:80` 的 `echo "Generating release notes via ${agent}..."` 写到 stdout，被 line 111 `> release_notes.txt 2>/dev/null` 一锅捕获；(b) AI 在响应里自述 "prompt cut off"（v2026.516.1 现场），因 prompt ≈14KB 经 `claude -p "$prompt"` 走 argv 触发截断/quoting 问题。**修法**：(1) 让 `_run_changelog_skill` 直接产出 `###` 分组格式（参照 v2026.514.3 历史样式）写入 CHANGELOG.md；(2) 删除 `_run_release_notes_skill` + `release_notes.txt` 整条路径，`.github/workflows/release.yml` 直接从 CHANGELOG.md `awk` 抽段（fallback 路径已存在，升为主路径）；(3) release.sh 状态 echo 全部改 `>&2`；(4) agent 调用改用 stdin 喂 prompt（写临时文件 + `<` 重定向 或 `printf '%s' \| <agent>`）防 argv 截断。**AC**：CLI `roll update` 与 GitHub release 同版本展示一致；新发版 `release_notes.txt` 不再生成（或释义掉）；发版流程少一次 AI 调用；历史 CHANGELOG.md 中扁平段落（.514.5 / .515.1 / .516.1）顺手回填为分组格式 — flagged by user 2026-05-16 | 📋 Todo |
