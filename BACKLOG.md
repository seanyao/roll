...
| FIX-024 | roll-build/roll-loop 在 CI 绿之前就标 ✅ Done — Phase 11 应在 `gh run watch` 确认当前 commit CI 通过后才写回 BACKLOG；需加 `roll ci [--wait]` 命令轮询 `gh run list` 状态，roll-loop 标完 Done 前强制调用；CI 红则保持 🔨 In Progress 并写 ALERT | 🔨 In Progress |
