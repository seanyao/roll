...
| FIX-019 | roll-.changelog 凭感觉填版本号 — loop 在 build/fix 后调用 changelog skill，claude 假设下一个版本号（v2026.511.8/9/11）写进 CHANGELOG.md，但 release.sh 才是版本号的唯一发布权威；导致 CHANGELOG 出现假节，release.sh 后还可能继续被绑架；roll-build/fix/changelog 改为只写 `## Unreleased` 节，release.sh 在拿到真实 N 后把 Unreleased 提升为 vX.Y.Z | 📋 Todo |
