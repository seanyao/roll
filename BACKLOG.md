...
| [US-AUTO-032](docs/features/autonomous-evolution.md#us-auto-032) | loop 在 worktree 里跑 — 每个 story 起 `~/.shared/roll/worktrees/<slug>-<US>` worktree + `loop/<US>` 分支隔离 TCR；**runner 起跑前先 `git fetch origin main --quiet`，worktree 直接基于 `origin/main` 而非 local main**（绕过 local main 落后/dirty 问题，loop 永远在最新远端基础上起跑）；成功 + CI 绿 → `git merge --ff-only` 回 main → push → 删 worktree+branch；失败保留 worktree + ALERT；plist/runner 路径不变（控制面常驻 main）；处理重试时分支已存在的幂等性；submodule 协同测试 | 📋 Todo |
...
| [US-AUTO-034](docs/features/autonomous-evolution.md#us-auto-034) | loop 扫开放 PR 自动跳过需人工介入的故事 — 任一开放 PR 的 title/body/headRef 含 story id 则本轮跳过该故事并写 ALERT；bin/roll 新增 `_loop_pending_pr_stories()` 用 `gh pr list --state open --json title,body,headRefName` 抽取 story id 集合（正则 `(US\|FIX\|REFACTOR)-[A-Z]+-[0-9]+`，gh 不可用则空集合 lenient）；SKILL Step 2 扫 BACKLOG 时过滤命中行，跳过的 story 写入 runs.jsonl `skipped` 数组（已有 schema）+ ALERT 一行；单测 `tests/unit/roll_loop_pr_skip.bats` 覆盖 3 路径（无 PR / PR-title 命中 / PR-branch 命中） `Domain: Autonomous Evolution > LoopRunner > PendingPRSet` | 📋 Todo |
