...
| FIX-028 | `roll peer` 结束时 cmd_peer 抛 "line 1737: resolution: unbound variable" — Resolution 已打印（值为 REFINE），但走到 REFINE/OBJECT case 内 `info "Peer requests $resolution. Continue to round $((round + 1))..."` 时崩；与 `set -euo pipefail` 互动诡异（变量明显已设置）。影响：peer 流程在第 1 轮 REFINE 时退出码 1（非预期 2），log file 仍能写入。修：复现 + 找出真正 unbound 变量（疑似 round 在某条件下未 local 化），加 unit 测试覆盖 REFINE/OBJECT/ESCALATE 三条退出路径 | 📋 Todo |
