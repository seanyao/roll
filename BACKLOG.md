...
| REFACTOR-010 | CI 测试三轮精简 Phase 2 — 找出慢测试，把文件结构检查类用例迁移到 `roll doctor`，目标减少 25% 用例数 | 🔨 In Progress |
| REFACTOR-011 | session 清理只删远端分支，本地 worktree 未 `git worktree remove` — `.claude/worktrees/` 长期积累，`git worktree list` 越来越嘈杂 — flagged by dream 2026-05-14 | 📋 Todo |
