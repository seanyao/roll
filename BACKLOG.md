...
| FIX-031 | loop LOCK 机制被并发打穿 — 2026-05-13 14:37 实测：runner.sh PID 78577 写进 `.LOCK-roll-d9dfa0` 且进程存活，但同 runner 下又起出第二个 claude 子会话同时操作 REFACTOR-009（详见 `~/.shared/roll/loop/ALERT.md` "concurrent invocation detected"）。当前 LOCK 仅校验 runner.sh 级（bin/roll:2084-2104），未约束 inner script / claude 子进程层；recovery / retry 路径若再次 spawn claude，会绕过外层 LOCK 引发并发改 state.yaml / BACKLOG / git。**排查方向**：(1) 14:12 那轮 runner 是否进过 rebase-abort 后的 cherry-pick recovery 分支，该分支可能直接 fork 新 claude 不走 LOCK；(2) tmux session 已存在时的 fallback 路径（bin/roll:2099）是否漏写 PID；(3) launchd 在 KeepAlive 重启场景下能否绕过 LOCK；(4) 给 inner script 加二级 LOCK（PID + start-ts 双校验）；(5) 加可复现的并发集成测试。`Domain: Autonomous Evolution > LoopScheduler > ConcurrencyGuard` | 📋 Todo |
| FIX-032 | roll-loop SKILL Step 2 解析 `depends-on:` / `manual-only:true` 标签 — 当前 BACKLOG 行末尾的 `` `depends-on:US-XXX` `` 与 `` `manual-only:true` `` 仅为约定，无强制力；loop 选 story 时不读这两个标签，可能在依赖未满足时领走 story（典型场景：US-AUTO-033 依赖 US-AUTO-037，loop 在 037 未完成时仍可能领 033）。修：bin/roll 加 `_loop_check_depends_on <story-id> <backlog-path>` (返回 satisfied/unsatisfied) + `_loop_is_manual_only <story-id>` 两个纯函数；SKILL Step 2 选 story 前调用，未满足依赖 → skip + 记入 runs.jsonl `skipped`；`manual-only:true` → skip + 记 INFO；单测覆盖 4 路径（无依赖 / 依赖 ✅ Done / 依赖 📋 Todo / manual-only）。**Loop-safe**：纯加法，不动 runner.sh。kimi peer review (2026-05-13) 指出此 parser 必须先于 Phase 2 落地，否则 depends-on 形同虚设。`Domain: Autonomous Evolution > LoopRunner > DependencyGate` | 📋 Todo |
...
| [US-AUTO-036](docs/features/autonomous-evolution.md#us-auto-036) | worktree 隔离 Phase 1：纯加法 helpers + 单测 — 在 bin/roll 加 `_worktree_path` / `_worktree_create` / `_worktree_cleanup` 等辅助函数；处理重试时分支已存在的幂等性；submodule 共存（main 与 worktree 同时 `tests/helpers/bats-core`）测试；新增 `tests/unit/roll_worktree.bats` 覆盖 7 路径（create / cleanup / retry-idempotent / ff-only-success / ff-only-failure / tcr-failure / fetch-failed-lenient）。**不动 runner.sh，不动 plist，不动 cron**——loop-safe，loop 自己可执行 `Domain: Autonomous Evolution > LoopRunner > Worktree` | 📋 Todo |
| [US-AUTO-037](docs/features/autonomous-evolution.md#us-auto-037) | worktree 隔离 Phase 2：runner 接入 + 集成验证 — `_write_loop_runner_script` 起跑时 `git fetch origin main --quiet`，`git worktree add ... -b loop/<US> origin/main`，cd 进 worktree 后跑 skill；完成后回 main `git pull --ff-only` + `git merge --ff-only loop/<US>` + push；失败保留 worktree + 写 `_LOOP_ALERT`；tmux session 命名加 story 后缀 (`roll-loop-<slug>-<US>`)。**改 orchestration、loop 不能自己跑**——必须 `$roll-build` 人工接管，验证含手动 `roll loop test` + 一轮真实 cron 触发 `Domain: Autonomous Evolution > LoopRunner > RunnerScript` `depends-on:US-AUTO-036` `manual-only:true` | 📋 Todo |
