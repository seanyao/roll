...
| [US-CL-005](docs/features/changelog-integration.md#us-cl-005) | changelog 风格守门 Phase 2 — stage 前自审 gate，5 条布尔 checklist 逐条判定，最多 3 轮重写后仍失败则 ALERT；自审日志写 jsonl 便于事后回看 agent 是否糊弄 `depends-on:US-CL-004` | ✅ Done |
...
| [US-AUTO-033](docs/features/autonomous-evolution.md#us-auto-033) | loop 自动建 PR + GitHub Auto-merge — story 完成后推 `loop/<US>` 分支，自动 `gh pr create` + `gh pr merge --auto --squash`，CI 绿自动合并，人零介入 `depends-on:US-AUTO-037` | ✅ Done |
...
| [US-AUTO-038](docs/features/autonomous-evolution.md#us-auto-038) | 清理孤立 `claude/*` session 分支 — Claude Code agent session 结束后遗留的 `claude/*` 分支推到 GitHub 但永远不会建 PR，导致 GitHub 首页持续显示 "Compare & pull request" banner 噪音；loop 每轮起跑时扫描并删除已合入 main 或超过 7 天无活动且无 open PR 的 `claude/*` 远端分支 `Domain: Autonomous Evolution > LoopRunner > BranchHygiene` `depends-on:US-AUTO-033` | 📋 Todo |
