...
| FIX-030 | loop 启动前不 fetch 远端，可能基于过期代码领取故事 — `roll-loop` Step 1 直接读本地 BACKLOG.md，跳过 `git fetch`；若用户/peer 推了新提交或改了 BACKLOG，loop 会在落后的 HEAD 上 build，TCR commit 串味、CI 红、甚至覆盖远端修改。修：bin/roll 新增 `_loop_precheck_remote()`（仿 `_loop_precheck_ci` bin/roll:2830 形式），`git fetch origin <branch> --quiet` 后比 `git rev-list --count HEAD..origin/<branch>`：落后=0 放行；落后>0 且 clean → `git pull --ff-only`；落后>0 且 dirty → ALERT 退出；fetch 失败（网络抖动）→ lenient return 0。SKILL.md 在现有 Step 1.5 之前加 Step 1.4 "Pre-run Remote Sync"。单测 `tests/unit/roll_loop_remote.bats` 覆盖 4 路径（in-sync / behind-clean / behind-dirty / fetch-failed）。`Domain: Autonomous Evolution > LoopRunner > PreflightGate` | 🔨 In Progress |
