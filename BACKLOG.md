...
| [US-AUTO-024](docs/features/autonomous-evolution.md#us-auto-024) | `roll loop runs` 每次 loop 运行的快速可见性 — 单次 loop 结束写 JSONL，新命令显示最近 N 次摘要 | 🔨 In Progress |
| [US-AUTO-025](docs/features/autonomous-evolution.md#us-auto-025) | loop 在 tmux session 里跑 + `roll loop attach` 实时观看 — 让 loop 从看不见的子进程变成可随时 attach 的活终端 | 📋 Todo |
