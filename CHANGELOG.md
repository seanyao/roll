...
### 交付方法
- 攻防结对(写测试和写实现由不同 agent 分工、绿后再轮流互相攻破)现在真正接进了循环引擎:verified/designed 档的卡按"另一个 agent 写红测试 → 实现者写代码变绿 → 攻击者轮流补破坏性测试直到攻不动"的顺序编排,回合数、攻破出的漏洞、终止原因(攻不动/回合上限/超时)都留下事件记录;standard 档完全不变(US-LOOP-102) `[loop-engine]`
