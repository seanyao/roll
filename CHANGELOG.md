...
- **builder git 操作钉到 cycle worktree,不再漏写共享 checkout**：pi/reasonix 等无可靠 cwd flag 的 agent 即使内部 shell 跳回主 checkout,spawn 也会清理继承的 `GIT_*` 并注入 cycle worktree 的 `GIT_DIR`/`GIT_WORK_TREE`,确保 `git add/commit/status` 操作仍落在隔离 worktree,避免主 checkout 混入其他卡的 TCR commit。(FIX-914) `[loop-engine]`
