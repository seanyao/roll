...
The script handles everything: symlinks `roll` into `~/.local/bin`, adds it to PATH, and runs `roll setup` (installs `~/.roll/` and syncs conventions + skills to all AI tools).
...
```bash
# 1. Install (also runs roll setup automatically)
./install.sh

# 2. New project — create workflow files (run from project root)
cd my-app
roll init

# 3. Optional: re-sync after editing ~/.roll/config.yaml
roll sync

# 4. Optional: tag commits with AI client name
roll hook install

# 5. Broke something? Force-rebuild from repo
roll reset
```
...
```
Global  ~/.claude/CLAUDE.md            ← user-owned; @roll.md appended by roll setup
...
```bash
./scripts/migrate-to-roll.sh --dry-run   # preview
./scripts/migrate-to-roll.sh             # apply
```

## Upgrading from cybernetix

```bash
./scripts/migrate-to-wukong.sh --dry-run   # preview
./scripts/migrate-to-wukong.sh             # apply
```
