...
**AC:**
- [ ] `_install_launchd_plists` 中，plist 内容变化 AND 服务已加载（`_launchd_is_loaded "$label"`）→ 自动 `launchctl unload` + `launchctl load`
- [ ] plist 内容不变（幂等场景）→ 不触发 reload，行为与现在相同
- [ ] 服务未加载时 → 仅写文件，不 load（由 `roll loop on` 负责首次加载，行为不变）
- [ ] 单元测试：`tests/unit/launchd.bats` 补覆盖"内容变更+已加载 → reload"和"内容不变 → 不 reload"两个分支

**Files:**
- `bin/roll` (`_install_launchd_plists`)
- `tests/unit/launchd.bats`
