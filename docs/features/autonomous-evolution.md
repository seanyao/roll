...
- [ ] 单测 `tests/unit/roll_loop_pr_inbox.bats` 覆盖：自有 PR 跳过 / 外部 PR 调 review / stale PR rebase 成功 / stale PR rebase 冲突 / gh 不可用 5 路径
...
- **吞噬人工修改窗口**：若人正在评审一个 PR，loop 又对它发起 review，可能误 approve。AC 应加入 "PR 上有 human review activity 时跳过" 的保险（参考 `gh pr view --json reviews`）
