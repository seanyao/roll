...
**AC:**
- [ ] runner script 启动时检测 `tmux` 是否可用
  - 可用 → 以 detached session 形式跑 claude；session 名 `roll-loop-<slug>`
  - 不可用 → fallback 到原 headless 模式（`claude -p ... >> log 2>&1`）
- [ ] tmux session 启动前 kill 同名旧 session（防止前一次崩溃残留）
- [ ] tmux session 内输出同时 pipe 到 `cron.log`（保留原有日志，不退步）
- [ ] runner 等待 tmux session 结束才退出（保证 LOCK trap 时序正确）
- [ ] 新命令 `roll loop attach`
  - 当前项目 session 存在 → `exec tmux attach -t roll-loop-<slug>`
  - 不存在 → 提示 "No running loop. Next fire at HH:MM" + 退出 1
- [ ] tmux 分离（Ctrl+B D）后 loop 继续跑，不受影响
- [ ] 测试：runner 模板含 tmux 检测和 fallback；attach 命令存在/不存在两个分支
