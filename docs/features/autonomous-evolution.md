...
**基于 `origin/main` 而非 local main**——runner 起跑前 `git fetch origin main`，worktree 直接从 `origin/main` 起新分支。这样 local main 的状态（落后多少、是否 dirty、有无 WIP）全部与 loop 解耦：loop 永远在「最新远端」基础上工作，用户在 local main 的任何操作（WIP、未推、未 commit）都不会影响 loop，反之亦然。这也是 FIX-030（独立的 pre-flight fetch + dirty check）被合并到本 Story 后整体作废的原因——worktree 隔离 + `origin/main` 基线，dirty 检查这一步成了无意义动作。

**Domain Model:**
- Context: Autonomous Evolution / Loop Execution
- Aggregate: LoopRunner owns [Worktree, StoryBranch]
- Events raised: [RemoteFetched] · [WorktreeCreated] · [StoryMerged] → main pushed · [WorktreeRetained] → ALERT
- Invariants:
  - main 工作树在 merge 那一刻之前永远不被 loop 修改
  - worktree 基线永远是 `origin/main`，不依赖 local main 状态
...
```
runner 起跑
  ├─ git fetch origin main --quiet  ← 拉远端最新，失败容忍（网络抖动）
  ├─ git worktree add ~/.shared/roll/worktrees/<slug>-<US> -b loop/<US> origin/main
  │   ├─ 基于 origin/main 而非 local main：local main 落后/dirty 都无关
  │   └─ 分支已存在（重试场景）→ 先 git branch -D loop/<US> 再 add
  ├─ cd worktree && git submodule update --init --recursive
  ├─ skill 执行（TCR 微提交全发生在 loop/<US> 分支）
  ├─ ┌─ 成功 + CI 绿
  │  │   ├─ cd main，git pull --ff-only origin main（同步 local main 到 origin）
  │  │   ├─ git merge --ff-only loop/<US>
  │  │   │   ├─ 成功 → git push → 删 worktree + 删分支
  │  │   │   └─ ff-only 失败（main 在 fetch 后又移动 / local main dirty）→ 保留 worktree，写 _LOOP_ALERT
  │  └─ 失败（TCR / CI 红）→ 保留 worktree + 分支 + log，写 _LOOP_ALERT
```

**AC:**
- [ ] runner 起跑前先 `git fetch origin main --quiet`，失败容忍（lenient return）—— 下一轮 cron 自然重试
- [ ] worktree **基于 `origin/main`** 创建：`git worktree add ~/.shared/roll/worktrees/<slug>-<US> -b loop/<US> origin/main`（**不**基于 local main，绕过 local 落后/dirty 问题）
...
- [ ] 成功 + CI 绿 → 回 main → 先 `git pull --ff-only origin main`（同步 local main）→ `git merge --ff-only loop/<US>` → push → 删 worktree + 分支
- [ ] ff-only 失败（fetch 后 main 又移动 / local main dirty）→ 保留 worktree + 分支；通过 `_LOOP_ALERT`（不是新 alerts.log）写入 worktree 路径与"请手动 merge"指令
...
- [ ] 单测：worktree create/cleanup/retry-idempotent/ff-only-success/ff-only-failure/tcr-failure/fetch-failed-lenient 共 7 路径
