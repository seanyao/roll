...
- [ ] `roll loop runs` PR URL 列 —— 拆到后续小故事（runs.jsonl 写入 + reader 同改，独立工作量）
...
- [ ] runs.jsonl 新加字段 `pr_processed`（deferred — 待 `roll loop runs` 读端协调后增量上线）
...
- [ ] 集成测试 `tests/integration/cmd_loop.bats` 加一个用例（deferred — 单测已覆盖 inbox 路由 + lenient 路径）
...
- [ ] 同步开 repo `required_pull_request_reviews=1`，使「CI 绿 + AI 评审通过」成为合并双门（须人工 PATCH branch protection — workflow ready；命令在 `roll setup` 末尾打印，由人工执行）
- [x] 文档：`docs/features/loop-pr-pipeline-plan.md` 写明路径 A → C 切换步骤
- [ ] 集成验证：开一个故意有问题的测试 PR，确认 AI request-changes 卡住 merge（pending — 等首个真实 PR 触发 workflow 后验证）
...
**AC:**
- [ ] `bin/roll` 新增 `_loop_self_heal_ci <story-id>` —— 增计数 + 输出失败日志摘要；超限或被旁路时退 1
- [ ] `bin/roll` 新增 `_loop_clear_heal_state <story-id>` —— 计数清零（loop reset / CI 转绿时调用）
- [ ] `roll loop reset` 子命令清理 `heal/` 目录
- [ ] `skills/roll-loop/SKILL.md` CI Gate 段落更新：CI 红 → 调 `_loop_self_heal_ci` → 允许则 `Skill("roll-fix")` 重试 → 仍红则 ALERT
- [ ] ALERT 文本附带 heal 历史（尝试次数 + 最后失败摘要前几行）
- [ ] `tests/unit/roll_loop_self_heal.bats` 覆盖：首次允许 / 第二次允许 / 超限拒绝 / `ROLL_LOOP_NO_HEAL=1` 拒绝 / `_loop_clear_heal_state` 清零 共 5 条
