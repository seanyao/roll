...
沿用 [US-AUTO-028](../../BACKLOG.md) 已在用的格式，在每行 BACKLOG 描述末尾加 `` `depends-on:US-XXX[,US-YYY,...]` `` 内联标签。spec 文档的 `**Dependencies:**` 段保留可读性叙述。

未来可由 `roll-loop` SKILL Step 2 解析这个标签，自动跳过未满足依赖的 `📋 Todo`——单独一条 FIX 即可实现。
...
| US-AUTO-033 | loop 建 PR + auto-merge | ✅ | US-AUTO-037 |
| US-AUTO-035 | claude-code-review 加 approve | ✅ | US-AUTO-033 (value) |
| US-AUTO-034 | PR Inbox 先消化再领 | ✅ | US-AUTO-033, US-AUTO-035 |
