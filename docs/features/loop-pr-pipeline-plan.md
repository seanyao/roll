...
| Phase 3 落地：claude-code-review.yml 接 approve/request-changes/uncertain 三态 + escape hatch | 2026-05-13 | US-AUTO-035 完成。workflow 升级到 `pull-requests: write`，prompt 写明三种决策；escape hatch：PR body `[skip-ai-review]` 或 commit msg `SKIP_AI_REVIEW` 触发 auto-approve（紧急 hotfix 通道，让 CI 单独把关）。**未做的最后一步**（由人工 PATCH 触发，工具不可代劳）：`gh api -X PATCH repos/seanyao/Roll/branches/main/protection -f required_pull_request_reviews.required_approving_review_count=1` —— 该命令打到位之前，AI approve 只是软门；打到位之后才是合并必经的硬门。`roll setup` 末尾会打印这条命令作为提醒。 |
