...
**Solution Design（方案设计）** — 架构设计和模块拆分写入 `docs/features/<feature>-plan.md`。Greenfield 项目的 Tactical Model（Aggregate、Entity、不变式、Domain Event）写入 `docs/domain/`。

**方案评审**（`$roll-peer`，可选）— 大范围需求在拆 Story 前，由 peer agent 审查完整方案。行为与方向评审一致。

**Split + Write** — 方案拆解为符合 INVEST 原则的 User Story，写入 `docs/features/<feature>.md`（含完整 AC），并在 `BACKLOG.md` 中追加索引行。用户确认后才触发 `$roll-build`。
