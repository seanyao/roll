...
graph TB
    ...
    subgraph "Loop B: Implementation & Iteration"
        B1["$roll-init<br/>工程脚手架"] --> B2["$roll-story-build<br/>TCR 驱动开发"]
    ...
    subgraph "Loop C: Observability & Maintenance"
        ...
        C2 -->|Yes| C3["$roll-bb-debug<br/>现场取证"]
        C3 --> C4["$roll-bb-analyzer<br/>根因诊断"]
        C4 --> C5["$roll-fix-build<br/>回归修复"]
        C5 --> C1
        C2 -->|No| C1
    end
...
2. **按 skill 逐个软链**：为 `~/.roll/skills/` 中每个 `wk-*` 技能单独创建符号链接，挂载到各 AI 客户端目录（`~/.claude/skills/wk-*`、`~/.gemini/skills/wk-*` 等）。用户已有的 skills 不受影响，Roll skills 独立并存。
...
**Git Hook（可选，`roll hooks install`）**
...
| TCR (Test && Commit ∥ Revert) | `$roll-story-build`：测试通过即提交，失败即回滚 |
...
`$roll-init` 采用 **Global + Template 合并策略**（见 2.2.3），不同项目类型的差异主要体现在**交互层**的形态：前端项目有组件树和路由层，CLI 项目有命令注册和交互式输入，纯后端服务则只有 API 接口层。以下分模板展示各自的完整结构：
...
**完整交付管线**——`$roll-story-build` 不止于本地测试通过。它要求完成端到端的交付链：
...
`$roll-init` 生成的 CI 配置包含两条相互独立的管线，对应不同的触发场景：
...
自动触发信号：检测到 Story 涉及认证/授权、支付/计费、数据完整性校验、复杂状态机、或历史高 Bug 模块时，`$roll-story-build` 自动将该 Action 路由到 `$roll-spar`。
...
| 数字取证 | `$roll-bb-debug`：自动化现场证据采集 |
| 根因分析 (RCA) | `$roll-bb-analyzer`：结构化诊断报告 |
...
**热点检测**——自适应权重：频繁失败的 Story 自动提升采样权重。巡检发现的问题自动创建 `FIX-XXX` Backlog 条目，交由 `$roll-fix-build` 处理。
...
### 5.4 根因诊断：`$roll-bb-analyzer`

消费 `$roll-bb-debug` 产出的诊断 JSON，按多维度进行结构化分析（内容状态、网络失败、DOM 渲染异常、性能瓶颈），输出诊断结论和修复建议。

> **场景（接上）**：`$roll-bb-debug` 对审计列表页面取证，Network 维度捕获到 `GET /api/audit` 返回 200 但 `timestamp` 字段值为 `null`；Console 维度同时出现 `[warn] AuditEvent serializer: missing timestamp`。
>
> `$roll-bb-analyzer` 消费诊断 JSON，定位根因：v1.3 的 ORM 升级引入了字段别名变更，序列化层未同步，导致 `created_at` → `timestamp` 的映射断裂。修复方向明确，交 `$roll-fix-build` 处理。

### 5.5 回归修复：`$roll-fix-build`

执行单一问题的修复，比 `$roll-story-build` 更轻量，但保持相同的质量要求：
...
> **场景（接上）**：`$roll-fix-build` 执行 FIX-012，修复范围严格限定为序列化层字段映射，同时添加回归测试（断言 `timestamp` 非空且为 ISO 8601 格式）。
>
...
- **多 Agent 协调成本**：`$roll-story-build` 会根据 Action 的依赖关系判断是否启动并行子 Agent，但跨 Agent 的状态同步与冲突处理目前依赖约定而非强制协议，在高并发场景下仍有协调开销。
...
| `$roll-init` | 初始化 | 项目名 | 标准化目录结构 + CI 配置 |
| `$roll-story-build` | 实现 | Story ID | 已部署代码 + 验证证据 |
| `$roll-fly-build` | 快速实现 | 一句话需求 | 自动拆分 → 交付 |
| `$roll-spar` | 防御性实现 | 功能描述 | 攻防测试套件 + 实现代码 |
| `$roll-fix-build` | 修复 | Fix ID | 修复代码 + 回归测试 |
| `$roll-sentinel` | 巡检 | 巡检策略 | 健康报告 / FIX 条目 |
| `$roll-bb-debug` | 调试 | URL | 诊断 JSON + 截图 |
| `$roll-bb-analyzer` | 诊断 | 诊断 JSON | 根因分析 + 修复建议 |
| `$roll-fetch` | 情报 | URL / 关键词 | 网页抓取、搜索、爬取结果 |
| `$roll-probe` | 监控 | 目标地址 | 节点发现、健康检查报告 |
...
| `roll sync conventions` | 可选：以 @include 模式追加 WK 约定（不覆盖现有文件） |
| `roll sync skills` | 刷新 skills 并修复 per-skill symlink |
| `roll sync all` | conventions + skills 一并执行 |
| `roll hooks install` | 可选：安装全局 git hook（需用户确认） |
| `roll init [type]` | 在当前目录生成约定文件（合并 Global + Template） |
