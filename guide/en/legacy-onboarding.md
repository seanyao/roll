...
This validates the paired diagnosis/plan artifacts, then:
...
Yes, but keep it paired with `.roll/init-diagnosis.yaml`. `roll init --apply` requires matching `factsHash` values, no shell-command keys, and only the two allowed `file_operations` entries. Plans older than 24 hours or generated by older `$roll-onboard` contracts should be regenerated.
