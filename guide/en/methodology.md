...
- **Graft** (existing code, no `.roll/`): surfaces `$roll-onboard`, which scans the code, asks a short clarification set, and writes `.roll/onboard-plan.yaml` for review; `roll init --apply` then prints a planned-operation checkpoint and waits for confirmation before writing — see [legacy-onboarding.md](legacy-onboarding.md).
