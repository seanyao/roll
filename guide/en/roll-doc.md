...
`roll-doc` scans any project root and produces draft documentation: a doc
index, module READMEs for undocumented directories, and — in Phase 3b — deep
cross-directory topic docs (data flows, state machines, integrations, and
more). It never overwrites human-authored files; it only writes drafts for a
human to review and commit.

```
$roll-doc              # full run (all phases)
$roll-doc --dry-run    # Phases 1–2 only; print the Phase 3 / 3b plan, write nothing
$roll-doc --force      # regenerate drafts even when target files already exist
```

## The Four-Phase Pipeline

roll-doc runs four phases in order, plus a deep-read Phase 3b that fires when
...
records — roll-doc never fabricates a line number.
