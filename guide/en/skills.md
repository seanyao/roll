...
2. Regenerate the capability catalog:
   ...
   says `GENERATED by roll skills generate — do not edit by hand`. Adding or
...
- `roll skills check` rescans and fails (non-zero exit, prints a diff) if
...
- `roll doctor` prints a non-failing nudge in its skills section when the
  catalog is stale, as a local reminder to run `roll skills generate`.
