...
```
.roll/verification/<story-id>/<run-id>/report.html   ← 报告（自包含）
.roll/verification/<story-id>/<run-id>/evidence.json ← 采集的硬事实
.roll/verification/<story-id>/latest                 ← 指向最新一次的软链
```

每次运行按时间戳落盘、永不覆盖。backlog 的 `✅ Done` 行链接到
`latest/report.html`；CHANGELOG 条目旁可带不可见的
`<!-- evidence: ... -->` 注释 marker 供追溯。

## 报告怎么产生

1. build/fix 的**验证门**阶段，agent 把原始输出 dump 到
   `.roll/verification/<id>/evidence/*.txt`，截屏放
