...
它自己的报告，不是全局 archive/index 页面。`roll attest` 只对当前故事负责：
它只写本故事的 run 报告和 `latest` 指针，不刷新任何全局 archive / 史诗 / 首页
HTML。那些归档页想看时用 `roll index` 按需渲染，它们是便捷/归档视图，不是交付真相面。
...
`roll index` 是按需的修复/归档渲染器。它把归档重建为可浏览的三层静态
...
`roll status`、`roll loop watch`、`roll loop runs`、`roll cycle <id>`。手动
运行 `roll index` 只用于对账、归档导出、CI artifact 或迁移修复；`--rebuild`
