...
| 分类 | 典型信号 | Supervisor 建议动作 |
|---|---|---|
| `auth_block` | "403"、"please run /login"、"Unauthorized" | `pause_for_owner` |
| `network_block` | `ECONNREFUSED`、`ETIMEDOUT`、DNS 失败 | `continue`（瞬态；loop 会重试或呼吸） |
| `setup_skill_root_pollution` | Reasonix 辅助目录警告、skill 缺少 description | `create_fix` → delta team |
| `worktree_permission_failure` | worktree 路径上的 `EACCES` / "permission denied" | `pause_for_owner` |
