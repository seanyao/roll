...
```
活跃窗口：上午 10 点 — 下午 6 点（用 `roll config loop-window 10-18` 设置）
```
...
Instead of hand-editing yaml files, use the `roll config` family — each write also reloads the launchd plist automatically.
...
**读当前值（Reading the current value）。** Run any facade with no value to print the effective combination and its source.

任何 facade 不带值跑一次，就打印当前生效组合和来源：
...
**范围校验（Range validation）。** Out-of-range or non-numeric input is rejected with a bilingual error and exit code 2.

超界或非数字输入会被拒绝，报中英双行错误并以退出码 2 退出——如 `roll config loop-window 9-25` 打印 `loop-window end must be <= 24` / `loop-window 结束时间必须 ≤ 24`。

**`--global` vs `--project`。** Writes default to `--project` (`.roll/local.yaml`); pass `--global` to write `~/.roll/config.yaml`.

写入默认 `--project`（`.roll/local.yaml`，仅当前项目）。加 `--global` 写 `~/.roll/config.yaml`，作为所有没有项目级覆盖的项目的默认值。
...
**自动 reload（Auto-reload）。** After writing a schedule key, `roll config` re-installs the launchd plists so the change fires on the next window; if reload fails the yaml is still source of truth — run `roll loop on` to apply by hand.

写完调度 key 后，`roll config` 自动重装 loop / dream / brief 的 launchd plist，下个窗口即生效。reload 失败时（如沙箱里）yaml 仍是真相——跑 `roll loop on` 手工补刀。完整 key 列表与范围见 `roll config --help`。
