...
启动第一个 builder cycle 前，`roll loop go` 还会检查未确认的 bootstrap 产物，例如
`AGENTS.md`、`.claude/` 或 `.roll/` 元数据。如果当前脏文件只有这些产物，goal 会以
`bootstrap_artifacts_unconfirmed` 暂停，并打印待确认文件。确认归属后，把它们提交到正确仓库、
按项目策略外置/ignore，或清理掉；再重新运行 `roll loop go`。这个 preflight 不会启动 cycle，
也不会累加 no-progress。
