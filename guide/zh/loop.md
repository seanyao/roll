...
roll loop agent-routes show          # 打印当前生效的 .roll/agent-routes.yaml（US-AGENT-002）
roll loop agent-routes lint          # 校验 schema，错误带行号
roll loop agent-routes path          # 打印当前生效的路由文件路径
```

## Agent 智能路由（US-AGENT-001..011）

Loop 不再用单一固定的 `primary_agent` 跑所有 cycle。每个 Todo 在 roll-design
拆分时写入 **Agent profile**（`est_min` / `risk_zone` / `chain_depth`），再加
上项目根目录的 `.roll/agent-routes.yaml`，决定本轮 cycle 用哪个 agent×model。

`roll-design` writes per-story est_min / risk_zone / chain_depth fields on
split; `.roll/agent-routes.yaml` declares each agent's capability range and
history thresholds; before each cycle loop reads both to pick the routing.

两层决策：

1. **硬规则** — 故事必须落在 agent 的 `types` / `est_min` / `risk` 区间内，
   按声明顺序第一个命中的 agent 胜出。
2. **软偏好** — 当硬规则有 ≥ 2 个 agent 命中，回看 `runs.jsonl` 近
   `history.window_cycles` 条记录按 (agent, story_type) 聚合命中率；
   命中率 ≥ `history.prefer_threshold` 且样本 ≥ 5 的 agent 优先。

Each cycle's cron log emits one announcement line:
...
```
roll loop agent-routes show          # 当前配置
roll loop agent-routes lint          # 校验
```

若没有 agent 命中硬规则，loop 回退到 `history.cold_start_default`，并在 cron
log 写一行 WARN。
