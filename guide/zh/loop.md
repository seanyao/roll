...
roll loop repair-evidence <story-id>          # 干跑：展示 green manual-merge PR 因缺少证据而被阻塞的恢复事实
roll loop repair-evidence <story-id> --apply  # 记录 evidence:repair 事件，使该 PR 可被合并
...
- **`verified` = execute → evaluate** —— 用户可见行为、需截图/视觉证据，或历史证据
  ...
  三个分开的维度。
- **`planned` = plan artifact → execute → evaluate** —— 风险是"做错事"：需求不清、跨模块、
  或触及 truth/release/路由/状态语义。plan artifact 先写契约再进入 execute；evaluate 做
  planned-vs-delivered 映射。evaluate → execute 的修复回合受硬熔断约束（最大轮数、
  重复 finding 签名、预算、超时），触界即升级。

角色之间只通过 artifact 交接（planner 契约、builder 证据、eval-report），绝不共享原始
会话。跨 Story 的项目级协调（排序、冲突、预算、发布就绪）归 **Prime Agent**
（`roll supervisor`），不属于任何单张 Story 的执行。

当 owner 要求清空 backlog 时，Prime Agent 使用 backlog-clearing standard，而不是只看
...
owner 处理 `roll supervisor status/next/why` 给出的行动。若某张 green manual-merge PR
只是缺少交付证据（acceptance report / ac-map），可用
`roll loop repair-evidence <story-id> --apply` 记录一次 `evidence:repair` 事件，
`roll supervisor next` 会报告 `merge_ready`，PR loop 随后将其合并。
