...
roll loop watch       # 只读、精炼、实时地查看本项目 loop（把 live.log 接入渲染器）
...
（`IDEA` 或 `FIX`，标 `📋 待人确认`），并由 `roll loop signals`
汇总报告。信号按
...
ALERT 条目会在 `roll loop status`、`roll loop alert` 和 `roll loop signals` 输出中显示。
...
plist——旧版本退役的形态曾以僵尸身份指着已删除的引擎存活数周。
