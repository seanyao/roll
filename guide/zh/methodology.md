...
| Scrum Backlog | `BACKLOG.md` + `.roll/features/`：两层索引结构 |
...
**Split + Write** — 方案拆解为符合 INVEST 原则的 User Story，写入 `.roll/features/<feature>.md`（含完整 AC），并在 `BACKLOG.md` 中追加索引行。用户确认后才触发 `$roll-build`。
...
> **设计理念 — Markdown 即代码**：在 Roll 中，`BACKLOG.md` 和 `.roll/features/` 不是开发完成后生成的文档产物——它们是驱动开发的输入。Story 没有对应 Markdown 文件就等于不存在；Story 的 Verification Gate 证据未提交，就视为未完成。文件系统是唯一的数据源，不需要同步额外的项目管理工具。
...
├── BACKLOG.md           # 任务索引
└── .roll/features/       # Story 详情与设计文档
```

三个文件，5 秒内完成。之后再次执行 `roll setup` 即可把约定与技能分发到各 AI 工具配置。
...
| `$roll-design` | 设计 | 需求描述 | BACKLOG.md + .roll/features/ |
...
| `roll init` | 在项目目录创建 AGENTS.md + BACKLOG.md + .roll/features/；已有 AGENTS.md 则重新合并 |
