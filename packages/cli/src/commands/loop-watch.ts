...
function watchHelp(): string {
  return [
    ...
    "",
    "It NEVER writes to or signals a running cycle — watching can never affect the loop. Ctrl-C ends the view, not the loop.",
    "它绝不写入或干预运行中的 cycle——观察永远不会影响 loop。Ctrl-C 只结束视图，不会停掉 loop。",
    "",
    "Options:",
