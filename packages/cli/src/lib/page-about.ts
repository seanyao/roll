...
export function renderAboutPage(input: RenderAboutInput): string {
  ...
  const body =
    ...
    `<h1 style="${MACHINE_PAGE_H1_STYLE}">${bi("How roll works", "roll 怎么运转")}</h1>` +
    `<p style="${MACHINE_PAGE_LEDE_STYLE}">${bi(
      "The shared harness charter: feedback loop, capability domains, principles, and invariants that govern every roll project.",
      "共享 harness 章程：反馈闭环、能力域、理念与不变量，治理每个 roll 项目。",
    )}</p></div>` +
    creed +
