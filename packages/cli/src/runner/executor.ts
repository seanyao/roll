...
import { declaresAnySurface, deliverableCmdsForStory, physicalTerminalCaptureCommandForStory, readAttestGateMode, rejectedDeliverableCmdsForStory, rejectedPhysicalTerminalCmdsForStory, runAttestGate, screenshotExemption, storyRequiresScreenshot, storySpecPath, verificationReportHasContent, verificationReportPath, webCaptureTargetsForStory } from "./attest-gate.js";
    ...
    attest: {
      async render(projectCwd, storyId, runDir) {
        ...
        try {
          ...
          const physicalCommand = physicalTerminalCaptureCommandForStory(projectCwd, storyId);
          const commands = [...deliverableCmdsForStory(projectCwd, storyId)];
          if (physicalCommand !== null && !commands.includes(physicalCommand)) commands.push(physicalCommand);
          ...
          const cmdSkipArgs = [...rejectedDeliverableCmdsForStory(projectCwd, storyId), ...rejectedPhysicalTerminalCmdsForStory(projectCwd, storyId)].flatMap((c) => [
            "--capture-command-skip",
            `terminal capture command 非白名单(仅限 roll 只读子命令): ${c}`,
          ]);
