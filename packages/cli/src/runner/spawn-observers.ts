...
// ── US-CYCLE-002: per-role spawn-watchdog caps ─────────────────────────────
...
export async function spawnWatched<T>(opts: {
  ...
  const pollMs = Number((process.env["ROLL_TIMEOUT_POLL_MS"] ?? "").trim()) || TIMEOUT_POLL_MS;
  ...
  const handle = watchRun({
    ...
    onTimeout: (info) => {
      ...
      // screenshot line). Best-effort — never let a write toppling the cycle.
      try {
