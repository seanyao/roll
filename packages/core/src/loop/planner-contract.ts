 ...
 *
 * This module owns the pure render/parse/validate logic + the planned-vs-delivered
 * mapping. A missing or malformed planner contract FAILS CLOSED before the Builder
 * starts (the runner gate). The Planner stays a skill; TS validates its artifact.
 */
import type { PlannedVsDeliveredRow, PlannerContract } from "@roll/spec";
...
/** Render a {@link PlannerContract} to `planner-contract.md` (round-trips). */
export function renderPlannerContract(c: PlannerContract): string {
  const parts = [
    `# Planner contract — ${c.storyId}`,
    "",
    H.scope,
...
/**
 * Parse `planner-contract.md` back into a {@link PlannerContract}. Returns null
 ...
 */
export function parsePlannerContract(md: string, storyId: string): PlannerContract | null {
...
/**
 * US-V4-006 — validate the PLANNER artifact pair (manifest + planner-contract).
 * Fail-closed BEFORE the Builder runs: the manifest must be a well-formed planner
...
export function validatePlannerArtifact(opts: {
  ...
  const man = validateArtifactManifest(opts.manifest, "planner");
  ...
  if (contract === null) {
    ...
    reasons.push("planner contract has no acceptance items (empty contract)");
  }
  return { ok: reasons.length === 0, reasons };
}

/**
 * US-V4-006 — map the planner's acceptance contract against what was delivered.
...
/** A one-line human summary of a planned-vs-delivered mapping (for the eval report). */
export function summarizePlannedVsDelivered(rows: readonly PlannedVsDeliveredRow[]): string {
  if (rows.length === 0) return "";
  const counts = { satisfied: 0, changed: 0, missing: 0 };
  for (const r of rows) counts[r.status] += 1;
  return `planned ACs: ${counts.satisfied} satisfied, ${counts.changed} changed, ${counts.missing} missing`;
}
