...
export function collectDossierState(
  ...
  // 3. Collect truth board (audit, cycle, release) — degrade on failure
  let truth: TruthBoardInput;
  try {
    truth = collectTruthBoard(cwd, nowSec);
  } catch {
    truth = { generatedAt: iso(nowSec) };
  }

  // 4. Collect loop heartbeat — degrade on failure
  let loop: TruthSnapshotLoop | undefined;
  try {
    loop = collectLoopHeartbeat(cwd);
  } catch {
    /* best-effort — no loop data is honest */
  }
