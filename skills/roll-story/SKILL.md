...
- Single bug / hotfix / small change (use `roll-fix`)
- Only a vague one-line requirement with no US yet (use `roll-fly`)
...
### 3. Define verification
   ...
   - reference `$roll-.qa` for test pyramid strategy (unit/E2E/visual/smoke)
...
- Unit tests for: {logic components} - see `$roll-.qa` Unit Tests section
- E2E tests for: {user flows} - see `$roll-.qa` E2E Tests section
- Visual regression for: {UI stability} - see `$roll-.qa` Visual Regression section
...
- Use `roll-fix` when the work is a single small shipped fix, hotfix, or enhancement and backlog tracking is unnecessary.
