...
## Features by Epic
### Foo
- [alpha](docs/features/alpha.md) — A planning feature
- [beta](docs/features/beta.md) — A shipped feature
EOF
  _enforce_planning_markers "$TMP/features.md" "$TMP/BACKLOG.md"
  grep -qF "alpha](docs/features/alpha.md) — A planning feature *(规划中)*" "$TMP/features.md"
  ! grep -qF "beta](docs/features/beta.md) — A shipped feature *(规划中)*" "$TMP/features.md"
}
...
EOF
  cat > "$TMP/features.md" <<'EOF'
- [delta](docs/features/delta.md) — description *(规划中)*
EOF
  _enforce_planning_markers "$TMP/features.md" "$TMP/BACKLOG.md"
  local count
...
EOF
  cat > "$TMP/features.md" <<'EOF'
- [shipped](docs/features/shipped.md) — done
EOF
...
EOF
  cat > "$TMP/features.md" <<'EOF'
- [epsilon](docs/features/epsilon.md) — being built
EOF
  _enforce_planning_markers "$TMP/features.md" "$TMP/BACKLOG.md"
...
@test "release.sh wires _enforce_planning_markers after AI features rewrite, before commit" {
  ...
  local rewrite_line; rewrite_line=$(grep -n 'docs/features.md updated' "$RELEASE_SH" | head -1 | cut -d: -f1)
