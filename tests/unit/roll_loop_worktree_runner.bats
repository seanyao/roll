...
@test "_write_loop_runner_script: attach.command pauses for user before closing (preserves scrollback)" {
  # The auto-popup attach script (generated by the OUTER runner, not the
  # inner) used `exec tmux attach`, which replaces the wrapping shell so
  # the Terminal window closes the instant the tmux session ends
  # (cycle_end kills the session). The entire scrollback disappeared with
  # the window, leaving no way to review what the cycle did unless the
  # user opened cron-<slug>.log.
  #
  # New behaviour: drop `exec`, prompt + `read` after `tmux attach` so
  ...
  # New form: bare attach + prompt + read.
  grep -qF 'tmux attach -t' "$script"
  grep -qF 'press enter to close' "$script"
  grep -qF '\nread _\n' "$script"
}
